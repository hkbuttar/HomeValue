...
export default function NeighborhoodsPage() {
  ...
  return <section className="shell"><div className="page-intro"><p className="eyebrow">Neighborhood dynamics</p><h1>Market archetypes,<br/>not stereotypes.</h1><p>Segments are named only after their measured price, growth, turnover, housing, income, and accessibility profiles are fitted.</p></div>
    {error ? <ErrorState message={error}/> : !records ? <LoadingState/> : <div className="segment-grid">{records.map((record, index) => <article key={index}><span>Cluster {String(record.cluster ?? index + 1)}</span><h2>{String(record.archetype ?? "Market segment")}</h2><dl><div><dt>Median sale</dt><dd>{record.median_sale_price ? `$${Number(record.median_sale_price).toLocaleString()}` : "—"}</dd></div><div><dt>Annual velocity</dt><dd>{record.annual_sale_velocity ? Number(record.annual_sale_velocity).toFixed(1) : "—"}</dd></div></dl></article>)}</div>}
  </section>;
}
